<!DOCTYPE html>
<html>
<head>
    <title>Customer Review</title>
    <style>
       body 
       {
        background: linear-gradient(to bottom, #ffb3cc, #ffcccc);
       }
        table {
            border-collapse: collapse;
            width: 100%;
            color: #7a3e38;
            font-family: monospace;
            font-size: 25px;
            text-align: left;
        }
        th {
            background-color: #7a3e38;
            color: white;
        }
        tr:nth-child(even) {
            background-color: #f2f2f2;
        }
    </style>
</head>
<body>
    <h1>Customer Review</h1>

    

    <br>

    <table>
        <tr>
            <th>ID</th>
            <th>Name</th>
            <th>Review</th>
            <th>Actions</th>
        </tr>
        <?php
        // Database connection details
        $serverName = "localhost";
        $userName = "root";
        $password = "";
        $dbName = "dmwcw";

        // Connect to the database
        $conn = mysqli_connect($serverName, $userName, $password, $dbName);

        if (!$conn) {
            die("Connection failed: " . mysqli_connect_error());
        }

        // Handle form submission
        if (isset($_POST['name']) && isset($_POST['review'])) {
            $name = $_POST['name'];
            $review = $_POST['review'];

            // Insert the new review into the database
            if ($name && $review) {
                $stmt = $conn->prepare("INSERT INTO reviews (Name, Review) VALUES (?, ?)");
                if ($stmt === false) {
                    die("Error preparing statement: " . $conn->error);
                }
                $stmt->bind_param("ss", $name, $review); // Bind the name and review
                if ($stmt->execute()) {
                    echo "New review added successfully!<br>";
                } else {
                    echo "Error executing statement: " . $stmt->error;
                }
                $stmt->close();
            } else {
                echo "Name or review cannot be empty!<br>";
            }
        }

        // Fetch the latest review
        $sql = "SELECT ID, Name, Review FROM reviews ORDER BY ID DESC LIMIT 1"; // Latest record
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $ID = $row["ID"];
            $name = $row["Name"];
            $review = $row["Review"];

            // Display the latest record
            echo '<tr>
                <td>' . $ID . '</td>
                <td>' . $name . '</td>
                <td>' . $review . '</td>
                <td>
                    <a href="Update.php?ID=' . $ID . '">Edit</a> | 
                    <a href="Delete.php?ID=' . $ID . '">Delete</a>
                </td>
            </tr>';
        } else {
            echo "<tr><td colspan='4'>No reviews found.</td></tr>";
        }

        // Close the database connection
        $conn->close();
        ?>
    </table>
</body>
</html>
